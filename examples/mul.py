#!/usr/bin/env python3
"""Standalone NV multiply example on RTX 3080 eGPU over PCIe/USB4.

Self-contained file: vendored NV stack (GSP boot, golden-image init, user
compute channel, GPFIFO, allocator) plus a hand-assembled SM86 SASS
cubin that computes out[i] = a[i] * b[i] for 4 floats.

Run:   python3 examples/mul.py
       -> result=[10.0, 40.0, 90.0, 160.0]

Tier 1 (offline):
       python3 examples/mul.py --middle-selftest
       -> mul_selftest=ok cubin_sha=... fmul=4 fadd=0

Live path imports only tinygrad.runtime.autogen (ctypes constants).
No Device["NV"], no _load_tinygrad, no from tinygrad.{device,runtime}.
"""
...
def build_cubin():
  bundles = [
    ...
    # SASS_ARITHMETIC: FMUL (multiply, not add)
    ((ch.Reg.R11 << 24) | (ch.Reg.R11 << 16) | ch.Op.FMUL, 0x00000007, 0x00000000, 0x004fe200),  # FMUL R11, R11, R7
    ((ch.Reg.R10 << 24) | (ch.Reg.R10 << 16) | ch.Op.FMUL, 0x00000006, 0x00000000, 0x004fe200),  # FMUL R10, R10, R6
    ((ch.Reg.R9  << 24) | (ch.Reg.R9  << 16) | ch.Op.FMUL, 0x00000005, 0x00000000, 0x004fe200),  # FMUL R9,  R9,  R5
    ((ch.Reg.R8  << 24) | (ch.Reg.R8  << 16) | ch.Op.FMUL, 0x00000004, 0x00000000, 0x004fe200),  # FMUL R8,  R8,  R4
...
MIDDLE_CUBIN_SHA256 = "7b32f4dcd913f7c602e0f10cbd9d73d111e7172bbbc85036fe1dd1c9807b49c6"  # FMUL cubin
...
def main():
  if "--middle-selftest" in sys.argv:
    middle_selftest()
    return
  # live path: do the actual multiply kernel
...
def middle_selftest():
  """Tier 1 offline gate (step 2+): cubin + launch words + helper sanity."""
  cubin = build_cubin()
  ...
  # FMUL-vs-FADD opcode check on the mul cubin
  text = cubin[ch.TEXT_OFF:ch.TEXT_OFF+512]
  fmul_count = sum(1 for i in range(0, 512, 16)
                  if (struct.unpack_from('<I', text, i)[0] & 0x7FFF) == ch.Op.FMUL)
  fadd_count = sum(1 for i in range(0, 512, 16)
                  if (struct.unpack_from('<I', text, i)[0] & 0x7FFF) == ch.Op.FADD)
  assert fmul_count == 4, f"expected 4 FMULs, got {fmul_count}"
  assert fadd_count == 0, f"expected 0 FADDs in mul cubin, got {fadd_count}"
  print(f"mul_selftest=ok cubin_sha={sha} launch_words={len(words)} fmul={fmul_count} fadd={fadd_count}")
