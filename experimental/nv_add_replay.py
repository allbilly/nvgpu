...
def patch_queue(queue, words, dev):
  queue._q = list(words)
  return queue


def main():
  parser = argparse.ArgumentParser(description="Replay or inspect captured tinygrad NV command words from dump/nv_add_dump.log.")
  parser.add_argument("--queue-submit", action="store_true", help="submit captured queue words through tinygrad NV queues; captured VAs are run-specific")
  ...
  if args.queue_submit:
    dev = Device["NV"]
    for event in events:
      queue_type = NVComputeQueue if event["queue"] == "NVComputeQueue" else NVCopyQueue
      patch_queue(queue_type(), event["words"], dev).submit(dev)
    dev.synchronize()
